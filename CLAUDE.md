...
日本の金融サービス（UFJダイレクト、楽天カード、Amazon Prime Mastercard）から取引データを取得し、Google Sheetsへ重複排除・カテゴリ分類付きで同期する個人財務自動化プロジェクト。
